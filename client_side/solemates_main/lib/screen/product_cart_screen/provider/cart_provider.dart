...
class CartProvider extends ChangeNotifier {
  ...
  checkCoupon() async {
    try {
      ...
      final response = await service.addItem(
        endpointUrl: 'couponCodes/check-doupon',
        itemData: couponData,
      );
      if (response.isOk) {
  ...
  addOrder(BuildContext context) async {
    try {
      // Debug: Print user ID to verify it exists
      print('User ID: ${_userProvider.getLoginUsr()?.sId}');

      // Debug: Print cart items
      print('Cart Items: ${myCartItems}');
      ...
      // Debug: Print complete order before sending
